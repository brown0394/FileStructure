...
{public:
	...
	int Read(int addr, RecType& record);
	...
	int Close ();
	const TextIndex& GetIndex();
	const int TextIndexedFile<RecType>::GetFirstIndex();
	const int TextIndexedFile<RecType>::GetNextIndex();
	void RewindDataFile();
...
template <class RecType>
int TextIndexedFile<RecType>::Read(int addr, RecType& record)
{
	if (addr < 0) return -1;
...
TextIndexedFile<RecType>::TextIndexedFile (IOBuffer & buffer,
	...
	:DataFile(buffer), Index (maxKeys), 
	 IndexBuffer(keySize, maxKeys), IndexFile(IndexBuffer)
{
	FileName = 0;
}
...
template <class RecType>
const TextIndex& TextIndexedFile<RecType>:: GetIndex() {
	return this->Index;
}

template <class RecType>
void TextIndexedFile<RecType>::RewindDataFile() {
	this->DataFile.Rewind();
}

template <class RecType>
const int TextIndexedFile<RecType>::GetFirstIndex() {
	return this->Index.FirstRecAddr();
}

template <class RecType>
const int TextIndexedFile<RecType>::GetNextIndex() {
	return this->Index.NextRecAddr();
}